    ...
    with torch.no_grad():
        for batch_idx in range(num_batch):
            if isinstance(model, torch.nn.Module):
                ...
                y = torch.from_numpy(valY[start_idx:end_idx]).float().to(device)
...
def train(model, trainX, trainTE, trainY, valX, valTE, valY, mean, std):
    ...
    for epoch in range(1, args.max_epoch+1):
        ...
        trainX = trainX[permutation].astype(float)
        trainY = trainY[permutation].astype(float)
        ...
        log_string(log, 'epoch %d, lr %.6f, train loss %.4f'
                   % (epoch, optimizer.param_groups[0]['lr'], train_l_sum / batch_count))

        # -------------------------------
        # 验证集 MAE，用 _compute_loss
        model.eval()
        with torch.no_grad():
            valX_tensor = torch.from_numpy(valX.astype(float)).float().to(device)
            valY_tensor = torch.from_numpy(valY.astype(float)).float().to(device)
            val_pred = model(valX_tensor, valY_tensor)
            val_loss = _compute_loss(valY_tensor, val_pred * std + mean).item()
        ...
        if val_loss < min_loss:
            ...
            print(f"Epoch {epoch}: Saved new best model with val_loss {val_loss:.4f}")
            no_improve_count = 0  # 重置计数器
        else:
            no_improve_count += 1  # 连续没改进的 epoch +1
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        #train(model, trainX, trainTE, trainY, valX, valTE, valY, mean, std)
