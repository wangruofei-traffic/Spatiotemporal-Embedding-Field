...
class STGNN(nn.Module):
    def __init__(self, infea, outfea, L, d, dev):
        ...
        # 流量特征映射
        self.flow_fc = nn.Linear(1, 32)

        # 输入嵌入
        self.start_emb = nn.Linear(64, outfea)
        self.end_emb = nn.Linear(outfea + 32, infea)

        # y 后32维映射为64维
        self.y_fc = nn.Linear(32, 64)

        # 模型主体
        self.stgnnwithgru = nn.ModuleList([STGNNwithGRU(outfea) for _ in range(L)])
        self.positional_encoding = PositionalEncoding(outfea)
        self.transform = nn.ModuleList([Transform(outfea, d) for _ in range(L)])
        self.L = L
    ...
    def forward(self, x, y=None):
        ...
        """
        # Step1: 流量+外部特征嵌入
        flow_feat = self.flow_fc(x[..., [0]])   # [B,T,N,32]
        ext_feat = x[..., 1:]                  # [B,T,N,32]
        x = torch.cat([flow_feat, ext_feat], dim=-1)  # [B,T,N,64]
        ...
        # Step3: Positional Encoding + Transformer
        ...
        x = torch.cat([x, y_feat], dim=-1) # 按时间步拼接

        # Step5: 输出
        x = self.end_emb(x)
        #print("x", x.shape)
        return x.squeeze(-1)  # [B,T,N] 或 [B,T,N,1]
