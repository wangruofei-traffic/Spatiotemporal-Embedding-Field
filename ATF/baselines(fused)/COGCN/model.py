...
class COGCN(nn.Module):
    def __init__(self,device, num_nodes, CL,l,dropout=0.3, supports=None,length=12, 
                 in_dim=64,out_dim=12,residual_channels=32,dilation_channels=32,
                 skip_channels=256,end_channels=512,kernel_size=2,K=3,Kt=3):
        ...
        self.bn_1=BatchNorm2d(in_dim,affine=False)
        self.flow_fc = nn.Conv2d(1, 32, kernel_size=(1, 1))  # 1维 → 32维
        # 标签属性映射: 32 -> 64
        self.future_attr_fc = nn.Conv2d(32, 64, kernel_size=(1, 1))

    def forward(self,input,label):
        #input: batch_size x channels x nodes x time_length
        shape=input.shape
        #print("input shape",shape)
        flow = input[:, 0:1, :, :]  # (B,1,N,T)
        flow_mapped = self.flow_fc(flow)  # (B,32,N,T)

        # 取后32维属性
        attr = input[:, 1:, :, :]  # (B,32,N,T)

        # 拼接形成64维
        input = torch.cat([flow_mapped, attr], dim=1)  # (B,64,N,T)
        ...
        # 取后32维未来属性
        future_attr = label[:, 1:, :, :]  # shape: (B, 32, N, T)

        # 映射到64维
        future_attr_mapped = self.future_attr_fc(future_attr)  # shape: (B, 64, N, T)
        #print("v",v.shape)
        #print("z",z.shape)
        #print("future_attr_mapped",future_attr_mapped.shape)

        # 与模型输出 x 融合
        x = torch.cat((v, z), 1)  # 原本 (B, 64, N, T)
        x = x + future_attr_mapped  # 逐元素相加
        #print("x",x.shape)
