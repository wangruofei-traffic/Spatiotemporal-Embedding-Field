...
class Model(object):
    ...
    def embeddings(self):
        ...
        :return:
        '''
        p_emd = embedding(self.placeholders['position'], vocab_size=self.site_num, num_units=self.para.emb_size+12*32, scale=False, scope="position_embed")
        p_emd = tf.reshape(p_emd, shape=[1, self.site_num, self.emb_size+12*32])
        ...
        h_emb = embedding(self.placeholders['hour'], vocab_size=24, num_units=self.para.emb_size+12*32, scale=False, scope="hour_embed")
        self.h_emd = tf.reshape(h_emb, shape=[-1, self.total_len, self.site_num, self.emb_size+12*32])

        m_emb = embedding(self.placeholders['minute'], vocab_size=24 * 60 //self.granularity, num_units=self.para.emb_size+12*32, scale=False, scope="minute_embed")
        self.m_emd = tf.reshape(m_emb, shape=[-1, self.total_len, self.site_num, self.emb_size+12*32])
    ...
    def model(self):
        ...
        features = tf.reshape(self.placeholders['features'], shape=[-1,
                                                                    ...
                                                                    self.site_num,
                                                                    self.features])
        labels = tf.reshape(self.placeholders['labels'], shape=[-1,
        ...
        #print("shapes",in_hour.shape,in_mimute.shape,in_position.shape)
        ...
        x = encoder_init.encoding(features,Y_attr)
        # decoder
