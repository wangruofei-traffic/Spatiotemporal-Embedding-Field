...
class DelaClass(object):
    ...
    def cnn(self, x=None):
        ...
        '''
        x_front = x[..., :1]  # 取第1个特征（shape [B, T, N, 1]）
        x_rest = x[..., 1:]  # 取剩下32维（shape [B, T, N, 32]）

        # 全连接层映射第1维 → 32维
        x_front_fc = tf.layers.dense(inputs=x_front,
                                     units=32,
                                     activation=tf.nn.relu,
                                     name='pre_fc_map')

        # 拼接 → [B, T, N, 64]
        x = tf.concat([x_front_fc, x_rest], axis=-1)
        print("After FC mapping, x:", x.shape)

        x = tf.transpose(x, perm=[0, 2, 1, 3])  # [batch size,  site num, input length, features]

        filter1 = tf.get_variable("filter1", [self.h, self.w, 64, 64],
    ...
    def encoding(self, x=None, y_future=None):
        """
        x: 输入序列, 形状 [B, input_length, site_num, features]
        y_future: 标签 (仅属性部分), 形状 [B, site_num, output_length, 33]
        """
        ...
        # ----------- 提取未来属性 -----------
        # y_future: (B, 66, 12, 33)
        future_attr = y_future[:, :, :, 1:]  # 取第2~33维属性 → (B, 66, 12, 32)
        #print("future_attr", future_attr.shape)
        future_attr = tf.reshape(future_attr, [-1, self.site_num, self.output_length * 32])  # (B, 66, 384)
        #print("future_attr", future_attr.shape)

        # ----------- 拼接未来属性到编码输出 -----------
        cnn_x = tf.concat([cnn_x, future_attr], axis=-1)  # (B, 66, 64+384)
        lstm_x = tf.concat([lstm_x, future_attr], axis=-1)  # (B, 66, 64+384)
        #print("cnn_x", cnn_x.shape)
        #print("future_attr", future_attr.shape)
    ...
    def decoding(self, x=None, embeddings=None, index_emb=None):
        ...
        with tf.variable_scope('decoder_dense', reuse=tf.AUTO_REUSE):
        ...
        pre = tf.multiply(tf.concat([
            tf.expand_dims(l_1, axis=-1),
            tf.expand_dims(l_2, axis=-1),
            tf.expand_dims(l_3, axis=-1)], axis=-1), W)
