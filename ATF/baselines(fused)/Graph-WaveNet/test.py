...
parser.add_argument('--checkpoint', type=str, default='./garage1/yinchuan', help='')
...
def main():
    ...
    for iter, (x, y) in enumerate(dataloader['test_loader'].get_iterator()):
        ...
        with torch.no_grad():
            preds = model(testx, testy).transpose(1, 3)
    ...
    np.savez_compressed('data/GraphWaveNet-' + 'YINCHUAN',
                        **{'prediction': scaler.inverse_transform(yhat).cpu().numpy(), 'truth': realy.cpu().numpy()})
