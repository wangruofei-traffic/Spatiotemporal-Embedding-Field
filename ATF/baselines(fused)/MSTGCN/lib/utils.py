...
def generate_data(dataset_dir):
    ...
    for category in ['train', 'val', 'test']:
        ...
        data['x_' + category] = np.transpose(cat_data['x'][..., :33], (0, 2, 3, 1))#修改处（取多维特征）
        data['y_' + category] = np.transpose(cat_data['y'][..., :33], (0, 2, 3,1))#修改处（取多维特征）
        # x = cat_data['x'][..., :1]
        # y = cat_data['y'][..., :1]
        if mean is None:
            # 只计算第1维（流量）的均值和标准差
            first_channel = data['x_train'][..., 0:1]  # shape: (samples, nodes, timesteps, 1)
            mean = first_channel.mean()
            std = first_channel.std()

    # ---------------------------
    # 🔹只对第一维归一化，其他维保持不变
    # ---------------------------
    def normalize_first_channel(x):
        x = x.copy()
        x[..., 0:1] = (x[..., 0:1] - mean) / std
        return x

    x_train = normalize_first_channel(data['x_train'])
    x_val = normalize_first_channel(data['x_val'])
    x_test = normalize_first_channel(data['x_test'])

    return x_train, data['y_train'], x_val, data['y_val'], x_test, data['y_test'], mean, std
        # yield (x - mean) / std, y
...
def load_data_new(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    ...
    val_target = val_target.astype(float)
...
def compute_val_loss_mstgcn(net, val_loader, epoch, limit=None):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            encoder_inputs, labels = batch_data, y_feat = batch_data

            outputs = net(encoder_inputs, y_feat)
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        prediction_length = prediction.shape[2]
        print('                MAE\t\tRMSE\t\tMAPE')
        data_target_tensor = data_target_tensor[:, :, 0, :]
        for (l,r) in [(0,66)]:
            for i in range(prediction_length):
