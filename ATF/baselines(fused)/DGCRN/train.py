...
parser.add_argument('--adj_data',
                    type=str,
                    default='data/sensor_graph/adj_yinchuan.pkl',
                    help='adj data path')
...
parser.add_argument('--save', type=str, default='./model.save-(1)/', help='save path')
    ...
    if args.TEST_ONLY:
        ...
        for i in range(1, args.epochs + 1):
            ...
            for iter, (x, y, ycl) in enumerate(
                ...
                x, y ,ycl = x.astype(float), y.astype(float),ycl.astype(float)
                batches_seen += 1
                trainx = torch.from_numpy(x).float().to(device).transpose(1, 3)
                trainy = torch.from_numpy(y).float().to(device).transpose(1, 3)
                trainycl = torch.from_numpy(ycl).float().to(device).transpose(1, 3)
            ...
            if (i - 1) % args.print_every == 0:
                ...
                log = 'Epoch: {:03d}, Train Loss: {:.4f}, Train MAPE: {:.4f}, Train RMSE: {:.4f}, test Loss: {:.4f}, test MAPE: {:.4f}, test RMSE: {:.4f}, Training Time: {:.4f}/epoch'
            ...
            if mvalid_loss < minl:
                ...
                minl = mvalid_loss
                print("Best model saved with MAE: %.4f" % minl)
                epoch_best = i
                count_lfx = 0
            else:
                count_lfx += 1
                print("No improvement in validation MAE. Wait: %d/%d" % (count_lfx, tolerance))
        ...
        outputs = []
        realy = torch.Tensor(dataloader['y_val'].astype(float)).to(device)
        realy = realy.transpose(1, 3)[:, 0, :, :]
