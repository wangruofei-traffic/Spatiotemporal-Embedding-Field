...
class Model(object):
    def __init__(self, para, min, max):
        ...
        self.placeholders = {
            ...
            'labels': tf.placeholder(tf.float32, shape=[None, self.site_num, self.total_len , self.para.features], name='labels'),
    ...
    def model(self):
        ...
        features = self.placeholders['features']  # 保持四维输入
        Y_attr = self.placeholders['labels'][:, :, -12:, :]
    ...
    def run_epoch(self,trainX, trainDoW, trainD, trainH, trainM, trainL, trainXAll, valX, valDoW, valD, valH, valM, valL, valXAll, testX, testDoW, testD, testH, testM, testL, testXAll):
        ...
        for epoch in range(self.epochs):
            ...
            print('validation')
            train_mae = self.evaluate(trainX, trainDoW, trainD, trainH, trainM, trainL, trainXAll)
            ...
            if mae < max_mae:
                ...
                print("Epoch %d: validation MAE improved to %.3f, saving model." % (epoch + 1, mae))
            else:
                wait += 1
...
def main(argv=None):
    ...
    print('trainX: %s\ttrainY: %s' % (trainX.shape, trainXAll.shape))
