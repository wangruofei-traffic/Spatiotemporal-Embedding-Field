...
class Trainer():
    ...
    def train(self, input, real_val, feat=None):
        ...
        if self.use_spatial:
            output, spatial_loss = self.model(input,real_val)
            real = real_val[:,:,:,0]
            predict = self.scaler.inverse_transform(output)
            loss = self.loss(predict, real, 0.0)-0.3*spatial_loss
        else:
            output, _ = self.model(input, real_val)
            real = real_val[:,0,:,:]
    ...
    def eval(self, input, real_val, flag='overall', return_data=False):
