...
def GMAN(X, Y,TE, SE, P, Q, T, L, K, d, bn, bn_decay, is_training):
    '''
    GMAN
    X：       [batch_size, P,
    ...
    # 将X的第一维特征33 -> 32
    X_first = X[..., :1]  # 假设第一维是第0个特征
    X_rest = X[..., 1:]   # 后32维特征

    # 用全连接层映射第一维特征 1 -> 32
    X_first_mapped = FC(
        X_first, units=[32], activations=[tf.nn.relu],
        bn=bn, bn_decay=bn_decay, is_training=is_training
    )  # [B, P, N, 32]

    # 拼接后形成64维特征
    X = tf.concat([X_first_mapped, X_rest], axis=-1)  # [B, P, N, 64]
    ...
    # 5. 融合未来属性
    Y_attr = Y[..., 1:]  # [B, N, Q, 32]
    #print("Y_attr",Y_attr.shape)
    # 转置成 [B, Q, N, 32] 与 transformAttention 对齐
    # [B, N, Q, attr_dim] -> [B, Q, N, attr_dim]
    Y_attr = tf.transpose(Y_attr, perm=(0, 2, 1, 3))
    Y_attr = FC(
        Y_attr,
        units=[D,D],  # D → D
        activations=[tf.nn.relu,None],  # 可选激活
        bn=bn, bn_decay=bn_decay,
        is_training=is_training
    )

    '''
    Y_attr = FC(Y_attr, units=D, activations=tf.nn.relu,
                bn=bn, bn_decay=bn_decay, is_training=is_training)
    #print("Y_attr", Y_attr.shape)
    '''
    # 可选：激活函数
    #Y_attr = tf.nn.relu(Y_attr)
    #print("Y_attr", Y_attr.shape)


    # 6. 相加融合

    #alpha_raw = tf.Variable(0.0, trainable=True, dtype=tf.float32)  # 原始变量
    #alpha = tf.sigmoid(alpha_raw)
    #X = tf.multiply(alpha, X) + tf.multiply(1.0 - alpha, Y_attr)
    X = tf.concat([X, Y_attr], axis=-1)
    X = FC(
        X,
        units=[D,D],  # 2D → D
        activations=[tf.nn.relu,None],  # 可选激活
    ...
    #拼接融合
    D = X.shape[-1]
    X_concat = tf.concat([X, Y_attr], axis=-1)  # [B, Q, N, 2D]
    W = tf.Variable(tf.random.normal([2 * D, D], stddev=0.1), name='fusion_W')
    b = tf.Variable(tf.zeros([D]), name='fusion_b')
    X = tf.tensordot(X_concat, W, axes=[[3], [0]]) + b  # [B, Q, N, D]
'''
    # decoder
    for _ in range(L):
